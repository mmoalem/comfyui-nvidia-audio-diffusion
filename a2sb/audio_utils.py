...
def wav_to_stft(wav: torch.Tensor, fft_size: int, hop_size: int, win_length: int, drop_dc_term=True):
    ...
    """
    stft_cmplx = torch.stft(wav, n_fft=fft_size, hop_length=hop_size, win_length=win_length,
                             window=torch.hann_window(win_length), return_complex=True)
    if drop_dc_term:
        stft_cmplx = stft_cmplx[..., 1:, :]

    magnitudes = stft_cmplx.abs()
...
def phase_R_to_channels(stft_R):
    ...
    if len(stft_R.shape) == 5:
        ...
        return phase_R_to_channels(stft_R.unsqueeze(-1))[0]
    else:
...
def stft_mag_R_to_wav(stft_mag, stft_Rch, n_fft, hop_length, win_length, append_dc_term=True):
    ...
    wav_out = torch.istft(new_stft_cmplx, n_fft=n_fft, hop_length=hop_length, win_length=win_length,
                          window=torch.hann_window(win_length).to(new_stft_cmplx.device))
    return wav_out
